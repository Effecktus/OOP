...
namespace Lab_3
{
    public partial class FormRhombus : Form
    {
        ...
        private void Create_Click(object sender, EventArgs e)
        {
            ...
            if (first && second && third && fourth && x>0 && y > 0 && x+w < pictureBox1.Width && y+h < pictureBox1.Height && w > 0 && h > 0)
            {
                gfx.Clear(Color.White);
        ...
        private void MoveTo_Click(object sender, EventArgs e)
        {
            ...
            if (Number.Text == "")
            {
                ...
                foreach (var item in rhombuses)
                {
                    item?.MoveTo(gfx, rand.Next(item.Width_Size + 15, pictureBox1.Width - (item.Width_Size) - 15),
                            rand.Next(item.Height_Size / 2 + 15, pictureBox1.Height - (item.Height_Size / 2) - 15));
                }
            }
            else if (first && second && third)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < rhombuses.Count(); i++)
                    {
                        ...
                        {
                            if (((newX - (rhombuses[i].Width_Size / 2) < 6) || (newX + (rhombuses[i].Width_Size / 2) > pictureBox1.Width - 6))
                                || ((newY - (rhombuses[i].Height_Size / 2) < 6) || (newY + (rhombuses[i].Height_Size / 2) > pictureBox1.Height - 6)))
                            {
                                MesBox("Введены неверные параметры, повторите ввод");
